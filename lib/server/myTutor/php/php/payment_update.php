...
$signed= hash_hmac('sha256', $signing, 'S-Z7M0kdg8TiQ37BFLgS9Gmg');
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        $sqlinsertpayment = "INSERT INTO `tbl_orders`( `email`,`receipt_id`, `order_paid`,`order_status`) VALUES ('$email','$receiptid','$amount','$status')";
        $sqpupdatecart = "UPDATE `tbl_carts` SET `cart_status`='paid',`receipt_id`='$receiptid' WHERE email='$email' AND cart_status IS NULL";
...
function printTable($receiptid,$name,$email,$amount,$paidstatus,$message){
   echo "
        ...
        <hr>
        <div class='w3-container w3-round w3-block w3-blue'>$message</div>
        </div>
        </body></html> 
        ";
}
