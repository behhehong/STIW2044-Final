...
$sqlinsert = "SELECT * FROM tbl_carts WHERE email = '$email' AND subject_id = '$subjectid' AND cart_status IS NULL";
...
if ($number_of_result > 0) {
	...
	$cartqty = $cartqty + 1;
	$updatecart = "UPDATE `tbl_carts` SET `cart_qty`= '$cartqty' WHERE email = '$email' AND subject_id = '$subjectid' AND cart_status IS NULL";
	$conn->query($updatecart);
} 
else 
{
    $addcart = "INSERT INTO `tbl_carts`(`email`, `subject_id`, `cart_qty`) VALUES ('$email','$subjectid','$cartqty')";
